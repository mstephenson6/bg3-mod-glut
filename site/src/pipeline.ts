 ...
 *   2. Runs every configured renderer against that same tree (in parallel).
 *   3. Copies any declared static-asset directories to their destinations.
 *
 ...
 *   2. **Render** — invoke every `Renderer` in the pipeline concurrently,
 ...
 *   3. **Assets** — copy each declared `{ src, dest }` pair recursively.
 ...
 *      concurrent copy.
 *
 * Errors from any renderer or asset copy are not swallowed — they propagate
 * out of `runPipeline` so the caller can decide how to handle them (e.g. fail
 * the build, log and continue, etc.).
 *
...
export const runPipeline = async (
  ...
  const { documentId, renderers } = pipeline;
  ...
  await Promise.all(renderers.map((renderer) => renderer.render(hast)));
